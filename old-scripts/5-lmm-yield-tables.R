...
for(crp in c('maize', 'beans', 'wheat', 'soybean', 'fababean')){
  ...
  fname <- paste0('pooled-site-random-anova-', crp, '.csv')
  ...
  fname <- paste0('pooled-site-interaction-anova-', crp, '.csv')
  if(fname %in% list.files(path=paste0(dir, 'output-data/models/'), pattern=".*csv$")){
    aov_interaction <- read.csv(paste0(dir, 'output-data/models/', fname)) 
  } else{ aov_interaction <- c() }  
  fname <- paste0('pooled-soil-interaction-anova-', crp, '.csv')
  if(fname %in% list.files(path=paste0(dir, 'output-data/models/'), pattern=".*csv$")){
  ...
  fname <- paste0('pooled-site-random-fe-', crp, '.csv')
  if(fname %in% list.files(path=paste0(dir, 'output-data/models/'), pattern=".*csv$")){
    fe_random <- read.csv(paste0(dir, 'output-data/models/', fname)) 
  } else{ fe_random <- c() }
  fname <- paste0('pooled-site-interaction-fe-', crp, '.csv')
  if(fname %in% list.files(path=paste0(dir, 'output-data/models/'), pattern=".*csv$")){
    fe_interaction <- read.csv(paste0(dir, 'output-data/models/', fname)) 
  } else{ fe_interaction <- c() }  
  fname <- paste0('pooled-soil-interaction-fe-', crp, '.csv')
  if(fname %in% list.files(path=paste0(dir, 'output-data/models/'), pattern=".*csv$")){
  ...
  fname <- paste0('pooled-site-random-re-', crp, '.csv')
  if(fname %in% list.files(path=paste0(dir, 'output-data/models/'), pattern=".*csv$")){
    re_random <- read.csv(paste0(dir, 'output-data/models/', fname)) 
  } else{ re_random <- c() }
  fname <- paste0('pooled-site-interaction-re-', crp, '.csv')
  if(fname %in% list.files(path=paste0(dir, 'output-data/models/'), pattern=".*csv$")){
    re_interaction <- read.csv(paste0(dir, 'output-data/models/', fname)) 
  } else{ re_interaction <- c() }  
  fname <- paste0('pooled-soil-interaction-re-', crp, '.csv')
  if(fname %in% list.files(path=paste0(dir, 'output-data/models/'), pattern=".*csv$")){
...
write.csv(all_crops_aov, paste0(dir, 'output-data/yields-aov.csv'), row.names=F)
write.csv(all_crops_fe, paste0(dir, 'output-data/yields-fe.csv'), row.names=F)
write.csv(all_crops_re, paste0(dir, 'output-data/yields-re.csv'), row.names=F)
